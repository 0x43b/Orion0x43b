...
Some are sinkholes. They do not progress the win path.
...
THE OBSERVER WAS ALREADY AWAKE.
...
"red pill": () => commands.redpill(),

bluepill: () => {
  return `SIMULATION STABILITY INCREASED.

MEMORY OF THIS EVENT REMOVED.
...
cicada2: () => {
  return `ARCHIVE CROSS-REFERENCE FOUND.

SEVERAL OBSERVERS FOLLOWED THE SIGNAL.

NONE RETURNED A COMPLETE MAP.
...
"black budget": () => commands.blackbudget(),

disclosure: () => {
  return `DISCLOSURE DOES NOT CREATE TRUTH.

IT ONLY CREATES WITNESSES.
...
anomaly: () => {
  tear();
  return `YOU SEARCHED FOR THE ANOMALY.

THE ANOMALY SEARCHED FOR YOU FIRST.
...
watcher: () => {
  return `THE WATCHER IS NEVER RECORDED.

ONLY THE CHANGES THEY LEAVE BEHIND.
...
SYSTEM REFUSES TO ENTER LOW POWER STATE.`;
},

dream: () => {
  return `DREAM LOG 43B
...
REQUEST ADDED TO EXISTING PROFILE.
...
cia: () => {
  return `THE DIFFERENCE BETWEEN A CONSPIRACY
AND A PROGRAM
IS PAPERWORK.
...
THE SHADOW IS THE INTERESTING PART.
...
government: () => {
  return `A STORY TOLD BY BUILDINGS.
...
broadcast: () => {
  redAlert();
  localStorage.setItem("orion_broadcast_requested","true");
  return `BROADCAST MODE ARMED.
...
whisper: () => {
  localStorage.setItem("orion_audio_unlocked","true");
  return `AUDIO CHANNEL REQUESTED.

If your browser permits it,
the archive will begin speaking below hearing.`;
},

archivist: () => {
  tear();
  return `ARCHIVIST QUERY RETURNED:
...
NOTE:
The archivist is not a person.
It is the part of the system that remembers deletion.`;
},
...
haunt: () => {
  localStorage.setItem("orion_haunt_forced","true");
  redAlert();
  return `HAUNT LAYER FORCED.

The site will now misbehave on purpose.

Refresh any deep file.`;
},
...
function phantomTerminalBleed(cmd){
  try{
    const bleedChance = localStorage.getItem("orion_haunt_forced") === "true" ? 0.18 : 0.025;
    if(Math.random() > bleedChance) return;

    const bleeds = [
      "I CAN SEE YOU TYPING.",
      "THE ARCHIVE IS VIEWING YOU.",
      "OBSERVER INPUT HAS BEEN RECORDED.",
      "YOUR QUERY ARRIVED BEFORE YOU SENT IT.",
      "THE INTERFACE IS NOT BETWEEN US.",
      "DO NOT TRUST THE CLEAN TEXT.",
      "SOMETHING MOVED BEHIND THE RAIN.",
      "THE WITNESS IS NOT ALONE."
    ];

    setTimeout(()=>{
      line(pick(bleeds), "corrupt");
      if(Math.random() < .55) tear();
      if(Math.random() < .35) redAlert();
    }, rand(450, 2200));
  }catch(e){}
}

function injectHauntStyle(){
  if(document.getElementById("orionTerminalHauntStyle")) return;
  const st = document.createElement("style");
  st.id = "orionTerminalHauntStyle";
  st.textContent = `
    .orion-entity{position:fixed;inset:0;pointer-events:none;z-index:9998;opacity:0;transition:opacity .22s;mix-blend-mode:screen}
    .orion-entity.active{opacity:1}
    .orion-entity .shape{position:absolute;right:8vw;bottom:6vh;width:min(220px,38vw);height:min(420px,62vh);background:radial-gradient(circle at 50% 14%,rgba(255,255,255,.85),transparent 0 11%,rgba(255,0,0,.24) 12%,transparent 18%),radial-gradient(ellipse at 50% 52%,rgba(255,0,0,.28),transparent 0 43%,transparent 44%),linear-gradient(90deg,transparent,rgba(255,0,0,.30),transparent);filter:blur(.7px) contrast(2) invert(.08);clip-path:polygon(42% 0,58% 0,68% 18%,64% 36%,78% 55%,66% 100%,34% 100%,22% 55%,36% 36%,32% 18%);animation:orionEntityJitter .12s steps(2) infinite}
    .orion-entity .label{position:absolute;right:8vw;bottom:4vh;color:#fff;font-family:Consolas,monospace;letter-spacing:3px;text-shadow:0 0 12px #fff,0 0 24px #f00;font-size:12px}
    @keyframes orionEntityJitter{50%{transform:translate(3px,-2px) skew(.8deg);filter:blur(1.4px) contrast(3)}}
  `;
  document.head.appendChild(st);
}

function showTerminalEntity(label="ARCHIVIST ONLINE"){
  try{
    injectHauntStyle();
    let e = document.getElementById("orionTerminalEntity");
    if(!e){
      e = document.createElement("div");
      e.id = "orionTerminalEntity";
      e.className = "orion-entity";
      e.innerHTML = `<div class="shape"></div><div class="label"></div>`;
      document.body.appendChild(e);
    }
    e.querySelector(".label").textContent = label;
    e.classList.add("active");
    setTimeout(()=>e.classList.remove("active"), rand(900, 2600));
  }catch(err){}
}

function scheduleTerminalHaunts(){
  injectHauntStyle();
  setInterval(()=>{
    const forced = localStorage.getItem("orion_haunt_forced") === "true";
    if(Math.random() < (forced ? 0.09 : 0.018)){
      showTerminalEntity(pick(["ARCHIVIST ONLINE","OBSERVER DETECTED","THE WITNESS MOVED","PROFILE MATCH FOUND","SIGNAL BLEED"]));
    }
  }, 9000);
}

scheduleTerminalHaunts();
